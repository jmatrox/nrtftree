...
{
    [TestClass]
    public class UnitTestSelectNodes
    {
        [TestMethod]
        public void TestMethodSelectChildNodesByType()
        {
            ...
            Assert.AreEqual(lista1.Count, 49);
            Assert.AreEqual(lista2.Count, 3);
            Assert.AreEqual(lista3.Count, 3);

            Assert.AreEqual(lista1[5], tree.MainGroup[8]);
            Assert.AreEqual(lista1[22].NodeKey, "lang");

            Assert.AreEqual(lista2[0], tree.MainGroup[45]); //'233
            Assert.AreEqual(lista2[1], tree.MainGroup[47]); //'241
            Assert.AreEqual(lista2[1].Parameter, 241); //'241

            Assert.AreEqual(lista3[0], tree.MainGroup[5]);
            Assert.AreEqual(lista3[0].FirstChild.NodeKey, "fonttbl");
            Assert.AreEqual(lista3[1], tree.MainGroup[6]);
            Assert.AreEqual(lista3[1].FirstChild.NodeKey, "colortbl");
            Assert.AreEqual(lista3[2], tree.MainGroup[7]);
            Assert.AreEqual(lista3[2].ChildNodes[0].NodeKey, "*");
            Assert.AreEqual(lista3[2].ChildNodes[1].NodeKey, "generator");
        }

        [TestMethod]
        public void TestMethodStyleSheet()
        {
            RtfTree tree = new RtfTree();

            int res = tree.LoadRtfFile("..\\..\\testdocs\\test.rtf");

            var x = tree.GetStyleSheetTable();

            Assert.IsTrue(tree.MainGroup.SelectSingleGroup("stylesheet").Rtf == x.ToString());
        }
    }
}
/*
